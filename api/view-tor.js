...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    ...
    try {
      ...
      console.log('🔍 DEBUG: View TOR request for ID:', tor_request_id);
      ...
      // Set headers for file viewing (inline) with correct file type
      const fileName = `TOR_${first_name}_${last_name}.${fileInfo.extension}`;
      res.setHeader('Content-Disposition', `inline; filename="${fileName}"`); // Changed from 'attachment' to 'inline'
      ...
      console.error('❌ ERROR in view-tor API:', error);
